...
public enum Prompts {
    ...
    public static func user(transcript: String, projectName: String?, projectContext: String,
        ...
        Respond with one JSON object of exactly this shape:
        {
          ...
          "action_items": [
            {"owner": "person's name, or null if unclear", "task": "one concrete next step, phrased as an imperative", "due": "deadline as mentioned (e.g. 'Friday', 'end of month'), or null", "due_date": "that deadline as a calendar date: 'YYYY-MM-DD', or 'YYYY-MM-DDTHH:MM' if a time of day was given; null when it can't be pinned to a specific day"}
          ],
          "open_questions": ["unresolved questions, blockers, or things that need follow-up"],
          "events": [
            {"title": "short name for something that will happen at a specific time (a follow-up meeting, call, demo, launch, review, appointment, trip, deadline)", "start": "'YYYY-MM-DD', or 'YYYY-MM-DDTHH:MM' when a time was given", "end": "'YYYY-MM-DDTHH:MM' if an end time or duration was given, else null", "all_day": "true when no time of day was given", "location": "place, room or video link if mentioned, else null", "context": "one sentence on what was said, e.g. 'Sarah proposed reviewing the mockups next Thursday afternoon'"}
          ]
        }
        Rules:
        ...
        - Dates: resolve relative expressions against the meeting date above. "Friday" or "this Friday" is the next Friday after the meeting day; "tomorrow" is the next day; "next week" needs a named day to count; "end of the month" is the last day of that month; "in two weeks" is 14 days later. All dates are in the meeting's time zone. Only fill in due_date or add an event when the day is clear from what was said — if it isn't, use null and no event. Never invent a time of day.
